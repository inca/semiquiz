...
Compiler.prototype = extend({}, BlockCompiler.prototype, {
  ...
  emitAssociativeGroup: function(walk) {
    ...
    for (i = 0; i < ctl.items.length; i++) {
      ...
      this.out.push(item.label);
      this.out.push('</label>');
      this.out.push('<div class="dropTarget"></div>');
      this.out.push('</div>');
    }
    // Include input for leftover items
    this.out.push('<input');
    this.emitAttrs({
      'id': item.id,
      'name': ctl.id,
  ...
  InlineCompiler: (function() {
    ...
    compiler.prototype = extend({}, InlineCompiler.prototype, {
      ...
      /**
       * The text input with default is mostly the same as text input, but has
       * default value at the beginning.
       *
       * The amount of empty space determines the size of the input.
       *
       * Example: `There {{   is   }[are]} an airplace in the sky.`
       */
      tryInputTextWithDefault: function (walk) {
        if (!walk.at('{{')) return false;
        var endIdx = walk.indexOf(']}');
        if (endIdx === null)
          return false;
      ...
      /**
       * Input regex starts with `{/` marker and ends with `/[flags][, size]}`.
       */
      tryInputRegex: function(walk) {
        if (!walk.at('{/')) return false;
        var endIdx = walk.indexOf('}');
        if (endIdx === null)
          return false;
        var source = walk.substring(walk.position, endIdx + 1);
        // Test against the regex
        var m = inputRegexRe.exec(source);
        if (!m) return false;
        // We got an input
        var ctl = this.formCompiler.createCtl('inputRegex');
        try {
          var flags = m[2] || undefined;
          ctl.value = new RegExp('^' + m[1].replace(/^\^/, '').replace(/\$$/, '') + '$', flags);
        } catch(e) {
          return false;
        }
        var size = m[3] && parseInt(m[3].substring(1).trim()) || m[1].length;
        this.out.push('<input');
        this.emitAttrs({
          id: ctl.id,
          name: ctl.id,
