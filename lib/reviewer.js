...
Reviewer.prototype.inputText = function(ctl, param) {
  ...
  if (diff.length > 1 || (diff[0].added || diff[0].removed)) {
    ...
    // Final cleanups
    this.$filled('#' + ctl.id + ' mark + ins').remove();
  }
};

Reviewer.prototype.inputRegex = function(ctl, param) {
  var actual = (param || '').toString().trim();
  this.input.push(actual);
  this.$review("input#" + ctl.id).val(actual);
  this.$filled("input#" + ctl.id).val(actual);
  if (!ctl.value.test(actual)) {
    this.errorIds.push(ctl.id);
    this.$review("input#" + ctl.id).addClass('error');
  }
};
...
Reviewer.prototype.associativeGroup = function(ctl, param) {
  ...
  param.forEach(function(id) {
    if (id == 'leftovers')
      return submittedCategories.push({
        item: { id: 'leftovers' },
        items: []
      });
  ...
  // Check the input
  var ctlHit = true;
  correctCategories.forEach(function(correctCategory) {
    var submittedCategory = _(submittedCategories).find(function(cat) {
      return cat.item.id == correctCategory.item.id;
    });
    // Category not submitted
    if (!submittedCategory) {
      ctlHit = false;
      correctCategory.items.forEach(function(item) {
        $review.find('#item-' + item.id).addClass('error');
      });
      return;
    }
    // Match items one by one
    submittedCategory.items.forEach(function(item) {
      this.input.push(submittedCategory.item.label + ': ' + item.label);
      var matches = _(correctCategory.items).findWhere({ id: item.id });
      if (!matches) {
        ctlHit = false;
        $review.find('#item-' + item.id).addClass('error');
      }
    ...
    // Match reverse
    correctCategory.items.forEach(function(item) {
      var matches = _(submittedCategory.items).findWhere({ id: item.id });
      if (!matches) {
        ctlHit = false;
        $review.find('#item-' + item.id).addClass('error');
      }
