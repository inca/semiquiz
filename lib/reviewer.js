...
Reviewer.prototype.sortableGroup = function(ctl, param) {
  ...
  param.forEach(function(id) {
    var $item = $filled.find('#item-' + id);
    $filled.append($item);
  });
  // Arrange items in `review` in correct order
  ctl.items.forEach(function(item) {
    var $item = $review.find('#item-' + item.id);
    $review.append($item);
  });
  // Check the input
  var ctlHit = true;
...
Reviewer.prototype.associativeGroup = function(ctl, param) {
  ...
  submittedCategories.forEach(function(category) {
    var $dropTarget = $filled
      .find('#category-' + category.item.id)
      .find('.dropTarget');
    category.items.forEach(function(item) {
      var $item = $filled.find('#item-' + item.id);
      $dropTarget.append($item);
    });
  });
  // Arrange items in `review` in correct order
  correctCategories.forEach(function(category) {
    var $dropTarget = $review
      .find('#category-' + category.item.id)
      .find('.dropTarget');
    category.items.forEach(function(item) {
      var $item = $review.find('#item-' + item.id);
  ...
  var ctlHit = true;
  submittedCategories.forEach(function(category) {
    var correct = correctCategories.find(function(cat) {
    ...
    category.items.each(function(item) {
      var matches = correct.items.findWhere({ id: item.id });
      if (!matches) {
        ctlHit = false;
    ...
    correct.items.each(function(item) {
      var matches = category.items.findWhere({ id: item.id });
      if (!matches) {
        ctlHit = false;
        $review.find('#item-' + item.id).addClass('error');
      }
